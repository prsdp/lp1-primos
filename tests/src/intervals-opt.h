...
void write_intervals(int li, int ls, double secs, int cont) {
...
void create_file() {
    ...
    fprintf(fp, "Segundos,Limite Inferior,Limite Superior,Quantidade Primos\n");
    fclose(fp);
}
...
void print_interval(int li, int ls){
    ...
    write_intervals(li,ls,(clock() - start) / (double)CLOCKS_PER_SEC,cont);//difftime(end,start));
